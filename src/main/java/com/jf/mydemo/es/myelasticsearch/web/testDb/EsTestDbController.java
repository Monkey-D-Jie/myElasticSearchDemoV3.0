package com.jf.mydemo.es.myelasticsearch.web.testDb;
...
import com.jf.mydemo.es.myelasticsearch.service.testDbService.TestDbEsService;
...
@RequestMapping("/myEsTestDbDemo")
public class EsTestDbController {
    /**
     * 记录日志信息
     */
    private Logger logger = LogManager.getLogger(EsTestDbController.class.getName());

    @Autowired
    private TestDbEsService testDbEsService;

    @RequestMapping(value = {"/fetchEmployeer"}, method = {RequestMethod.POST}, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public ResponseDataBean fetchEmployeer(@RequestParam("data") String data){
        ResponseDataBean responseDataBean = null;
        if(data != null){
            Map<String,Object> map = new HashMap<String,Object>(5);
            map.put("EmployeerId",data);
            RequestDataBean requestDataBean = new RequestDataBean();
            requestDataBean.setReqData(map);
            responseDataBean  = this.testDbEsService.fetchEmployeerById(requestDataBean);
        }
        return responseDataBean;
    }

    @RequestMapping(value = {"/insertOrModifyEmployeer"}, method = {RequestMethod.POST}, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public ResponseDataBean insertOrModifyEmployeer(@RequestParam("data") String data){
        ResponseDataBean responseDataBean = null;
        if(data != null){
            RequestDataBean requestDataBean = JsonXmlObjectKit.convertJsonObject(data,RequestDataBean.class);
            responseDataBean  = this.testDbEsService.insertOrModifyEmployeer(requestDataBean);
        }
        return responseDataBean;
    }

    @RequestMapping(value = {"/removeEmployeer"}, method = {RequestMethod.POST}, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public ResponseDataBean removeEmployeer(@RequestParam("data") String data){
        ResponseDataBean responseDataBean = null;
        if(data != null){
            Map<String,Object> map = new HashMap<String,Object>(5);
            map.put("EmployeerId",data);
            RequestDataBean requestDataBean = new RequestDataBean();
            requestDataBean.setReqData(map);
            responseDataBean  = this.testDbEsService.removeEmployeerById(requestDataBean);
        }
        return responseDataBean;
    }
     ...
     */
    /*, method = {RequestMethod.Employeer}, produces = {"application/json;charset=UTF-8"}*/
    /**
     * @author: wjie
     * @date: 2018/3/14 0014 13:35
     *
     * 在浏览器的url地址栏中访问的结果：
     * http://localhost:8080/myEsDemo/singleEmployeer?name=浣溪沙&size=20
     * 按照默认的排序方式，即匹配相关度排序，
     * 前10个匹配度最高，都是完全带“浣溪沙”三个字的。
     * 第10个、11个都是题目和正文都包含“溪”字而且出现了2次“溪”，最后一个是正文带一个“溪”。
     *
     * 在employee业务中，用的全匹配查询term
     */
    /**
     * @author: wjie
     * @date: 2018/3/29 0029 17:29
     *
     *用自定义的pagable接收前台穿过来的参数，并不能达到我想要的效果。
     * size和number的值都是null，连limit也塞不进去。。。
     * 似乎还是只有沿用原来的办法额。。。咱时还未找到 创建自定义pagable的法子
     *
     */
    @RequestMapping(value = {"/singleWord"})
    @ResponseBody
    public ResponseDataBean2 singleWord(String name, @Param("limit")Integer limit, @PageableDefault Pageable pageable){
        ResponseDataBean2 responseDataBean = null;
        if(name != null && pageable != null){
            MyPage page = new MyPage();
            page.setSize(limit);
            page.setNumber(pageable.getPageNumber());
            responseDataBean  = this.testDbEsService.fetchSingleTitle2(name,page);
        }
        if(this.logger.isInfoEnabled()){
            this.logger.info(JsonXmlObjectKit.convertObjectJson(pageable));
     ...
     * 这里在Dao层用的是 模糊查询(全字段)
     */
    @RequestMapping(value = {"/singleEmployeer"})
    @ResponseBody
    /**
     * 说明下：在@PageableDefault注解中说明的排序字段得是es的doc中的才行哈！
     */
    public ResponseDataBean2 singleEmployeer(String name, @Param("limit")Integer limit,@PageableDefault(sort = "birthDate", direction = Sort.Direction.DESC) Pageable pageable){
        ResponseDataBean2 responseDataBean = null;
        if(name != null && pageable != null){
            MyPage page = new MyPage();
            page.setSize(limit);
            page.setNumber(pageable.getPageNumber());
            responseDataBean  = this.testDbEsService.fetchSingleEmployeer2(name,page);
        }
        if(this.logger.isInfoEnabled()){
            this.logger.info("在EsController中的singleEmployeer，最终得到的返回数据为:<<---"+JsonXmlObjectKit.convertObjectJson(responseDataBean));
        }
        return responseDataBean;
    }
    /**
    ...
    @ResponseBody
    public ResponseDataBean singleMatch(String name, Integer empNo,@PageableDefault Pageable pageable){
        ResponseDataBean responseDataBean = null;
        //userId = 66;
        //content = "66";
        if(name != null && empNo != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchSingleMatch(name,empNo,pageable);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean singlePhraseMatch(String name,@PageableDefault Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(name != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchSinglePhraseMatch(name,pageable);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean singleTerm(Integer empNo,@PageableDefault Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(empNo != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchSingleTerm(empNo,pageable);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean multiMatch(String name, @PageableDefault(sort = "hireDate", direction = Sort.Direction.DESC) Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(testDbEsService != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchMultiMatch(name,pageable);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean contain(String name, @PageableDefault(sort = "hireDate", direction = Sort.Direction.DESC) Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(name != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchContain(name);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean contain2(String name, @PageableDefault(sort = "hireDate", direction = Sort.Direction.DESC) Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(name != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchContain2(name);
        }
        if(this.logger.isInfoEnabled()){
    ...
    @ResponseBody
    public ResponseDataBean bool(String name, Integer empNo, Date hireDate, @PageableDefault(sort = "hireDate", direction = Sort.Direction.DESC) Pageable pageable){
        ResponseDataBean responseDataBean = null;
        if(name != null && pageable != null){
            responseDataBean  = this.testDbEsService.fetchBool(name,empNo,hireDate);
        }
        if(this.logger.isInfoEnabled()){
